...
def apply_config_overrides(config, args):
    ...
    if hasattr(args, "model_dtype") and args.model_dtype is not None:
        config.model.dtype = cast_str_to_torch_dtype(args.model_dtype)
        overrides_applied.append(f"model.dtype = {args.model_dtype}")
