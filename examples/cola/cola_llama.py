...
_COLA_ACT_TRACE: Dict[str, torch.Tensor] = {}


def _cola_act_trace_enabled() -> bool:
    return os.environ.get("COLA_ACT_TRACE_ENABLE", "0").lower() in {"1", "true", "yes", "on"}


def _cola_act_trace_target_layer() -> int:
    return int(os.environ.get("COLA_ACT_TRACE_LAYER", "0") or "0")


def _cola_act_trace_capture(name: str, tensor: torch.Tensor, tp_pg: Optional[dist.ProcessGroup], gather_last_dim: bool) -> None:
    if not _cola_act_trace_enabled():
        return
    if name in _COLA_ACT_TRACE:
        return
    if not isinstance(tensor, torch.Tensor):
        return

    x = tensor.detach()
    if tp_pg is not None and tp_pg.size() > 1:
        # Only gather when this tensor is hidden-sharded across TP.
        if gather_last_dim:
            gathered = [torch.empty_like(x) for _ in range(tp_pg.size())]
            torch_dist.all_gather(gathered, x.contiguous(), group=tp_pg)
            x = torch.cat(gathered, dim=-1)
        # Keep only TP rank 0 local copy for tracing.
        if dist.get_rank(tp_pg) != 0:
            return

    _COLA_ACT_TRACE[name] = x.float().cpu().clone()


def _cola_act_trace_reset() -> None:
    _COLA_ACT_TRACE.clear()


def _cola_act_trace_maybe_save() -> None:
    if not _cola_act_trace_enabled():
        return
    out_dir = os.environ.get("COLA_ACT_TRACE_DIR", "").strip()
    if not out_dir:
        return
    if torch_dist.is_available() and torch_dist.is_initialized() and torch_dist.get_rank() != 0:
        return
    if len(_COLA_ACT_TRACE) == 0:
        return

    layer = _cola_act_trace_target_layer()
    save_once = os.environ.get("COLA_ACT_TRACE_SAVE_ONCE", "1").lower() in {"1", "true", "yes", "on"}
    os.makedirs(out_dir, exist_ok=True)
    path = os.path.join(out_dir, f"activation_trace_layer{layer}.pt")
...
class IdentityRMSNorm(nn.Module):
    def forward(self, x: Optional[torch.Tensor] = None, input: Optional[torch.Tensor] = None, **kwargs) -> torch.Tensor:
        if x is not None:
            return x
        if input is not None:
            return input
        raise ValueError("IdentityRMSNorm expects either `x` or `input` tensor.")
    ...
    def forward(self, hidden_states, s_local=None):  # [seq_length, batch_size, hidden_dim]
        ...
        if self.layer_idx == _cola_act_trace_target_layer():
            _cola_act_trace_capture(
                ...
                tensor=merged_states,
                tp_pg=self.tp_pg,
                gather_last_dim=False,
            )
        ...
        if self.layer_idx == _cola_act_trace_target_layer():
            _cola_act_trace_capture(
                name=f"layer{self.layer_idx}.after_gate_up_proj0_act",
                tensor=merged_states,
                tp_pg=self.tp_pg,
                gather_last_dim=False,
            )
        ...
        if self.layer_idx == _cola_act_trace_target_layer():
            # merged_states is [2, seq, batch, intermediate/TP] after batched column linear.
            # Gather the TP-sharded last dim for each branch.
            _cola_act_trace_capture(
                name=f"layer{self.layer_idx}.after_gate_up_proj1_batched",
                tensor=merged_states,
                tp_pg=self.tp_pg,
        ...
        if self.layer_idx == _cola_act_trace_target_layer():
            _cola_act_trace_capture(
                name=f"layer{self.layer_idx}.after_gate_mul_up",
        ...
        # [seq_length, batch_size, rank]
        hidden_states = self.down_proj0(hidden_states, rms_eps=self.rms_norm_eps)
        if self.layer_idx == _cola_act_trace_target_layer():
            _cola_act_trace_capture(
                name=f"layer{self.layer_idx}.after_down_proj0_linear",
                tensor=hidden_states,
                tp_pg=self.tp_pg,
                gather_last_dim=False,
            )
        ...
        if self.layer_idx == _cola_act_trace_target_layer():
            _cola_act_trace_capture(
                name=f"layer{self.layer_idx}.after_down_proj0_act",
                tensor=hidden_states,
                tp_pg=self.tp_pg,
                gather_last_dim=False,
            )
        ...
        if self.layer_idx == _cola_act_trace_target_layer():
            _cola_act_trace_capture(
                name=f"layer{self.layer_idx}.after_mlp",
                tensor=hidden_states,
                tp_pg=self.tp_pg,
                gather_last_dim=True,
            )
...
class CausalSelfAttention(nn.Module, AttachableStore):
    ...
    def forward(
        ...
        if self.layer_idx == _cola_act_trace_target_layer():
            _cola_act_trace_capture(
                name=f"layer{self.layer_idx}.after_qkv_proj0_act",
                tensor=qkv_states,
                tp_pg=self.tp_pg,
                gather_last_dim=False,
            )
        ...
        if self.layer_idx == _cola_act_trace_target_layer():
            _cola_act_trace_capture(
                name=f"layer{self.layer_idx}.after_core_attn",
        ...
        if self.layer_idx == _cola_act_trace_target_layer():
            _cola_act_trace_capture(
                name=f"layer{self.layer_idx}.after_o_proj",
                tensor=output,
                tp_pg=self.tp_pg,
                gather_last_dim=True,
            )
...
class LlamaDecoderLayer(nn.Module):
    def __init__(
        ...
        self.tp_pg = tp_pg
        self.config = config
        self.disable_all_rmsnorm = os.environ.get("COLA_DISABLE_ALL_RMSNORM", "0").lower() in {
            "1",
            "true",
            "yes",
            "on",
        ...
        if self.disable_all_rmsnorm:
            log_rank(
                "COLA_DISABLE_ALL_RMSNORM=1 -> decoder RMSNorms are identity.",
                logger=logger,
                level=logging.WARNING,
                rank=0,
            )
        ...
        if self.disable_all_rmsnorm:
            self.post_attention_layernorm = IdentityRMSNorm()
        elif use_sync_rmsnorm:
    ...
    def _core_forward(
        ...
        if self.layer_idx == _cola_act_trace_target_layer():
            _cola_act_trace_capture(
                name=f"layer{self.layer_idx}.input_x",
                tensor=hidden_states,
                tp_pg=self.tp_pg,
                gather_last_dim=True,
            )
        ...
        if self.layer_idx == _cola_act_trace_target_layer():
            _cola_act_trace_capture(
                name=f"layer{self.layer_idx}.after_attn_block",
                tensor=hidden_states,
                tp_pg=self.tp_pg,
                gather_last_dim=True,
            )
        ...
        if self.layer_idx == _cola_act_trace_target_layer():
            _cola_act_trace_capture(
                name=f"layer{self.layer_idx}.after_mlp_block",
                tensor=hidden_states,
                tp_pg=self.tp_pg,
                gather_last_dim=True,
            )
...
class Embedding(nn.Module, AttachableStore):
    ...
    def forward(self, input_ids: torch.Tensor, input_mask: torch.Tensor):  # [batch_size, seq_length]
        ...
        if self.pg.size() > 1:
            # We slice the hidden dim per-TP rank right after embedding. Since embedding forward
            # uses an all-reduce over TP, we must all-reduce gradients at this point so each vocab
            # shard receives contributions from all hidden chunks.
            input_embeds = differentiable_identity(input_embeds, group=self.pg)
...
class LlamaModel(nn.Module):
    ...
    def __init__(
        ...
        if self.disable_all_rmsnorm:
            log_rank(
                "COLA_DISABLE_ALL_RMSNORM=1 -> final RMSNorm is identity.",
                logger=logger,
                level=logging.WARNING,
                rank=0,
            )
    ...
    def forward_with_hidden_states(
        ...
        if _cola_act_trace_enabled():
            _cola_act_trace_reset()
...
class ColaLlamaForTraining(NanotronModel):
    ...
    def forward(
        ...
        if os.environ.get("COLA_STRICT_TENSOR_DUMP_INCLUDE_LOGITS", "0").lower() in {"1", "true", "yes", "on"}:
            return {"loss": loss, "sharded_logits": sharded_logits}
        return {"loss": loss}
    ...
    def get_cola_std(self, module: nn.Module, module_name: str) -> Optional[float]:
        ...
        if is_down_proj:
            ...
            if "gate_up_proj0" in module_name or "gate_proj0" in module_name or "up_proj0" in module_name:
                ...
                if "gate_up_proj0" in module_name:
                    expected_out_features = 2 * self.config.mlp_rank
                    assert (
                        out_features == expected_out_features
                    ), f"{module_name}.out_features={out_features} but expected {expected_out_features}"
                ...
                if "qkv_proj0" in module_name:
                    expected_out_features = 3 * self.config.attn_rank
                    assert (
                        out_features == expected_out_features
                    ), f"{module_name}.out_features={out_features} but expected {expected_out_features}"
