...
# at fixed micro-batch-size=4 for:
#   3B: TP=2
#   7B: TP=4
#
# Outputs:
#   - logs:    .logging/iter_comp_mbz4/*.log
#   - configs: .logging/iter_comp_mbz4/configs/*.yaml
#   - table:   .logging/iter_comp_mbz4/iter_compare_mbz4_table.txt
...
OUT_DIR=".logging/iter_comp_mbz4"
...
set_recompute_layer() {
  ...
  python - "$in_cfg" "$out_cfg" "$value" <<'PY'
import pathlib
import re
import sys

in_cfg, out_cfg, value = sys.argv[1:]
text = pathlib.Path(in_cfg).read_text()
pattern = r"(^\s*recompute_layer:\s*).*$"
out, count = re.subn(pattern, rf"\g<1>{value}", text, flags=re.MULTILINE)
if count == 0:
    raise RuntimeError(f"Could not find key 'recompute_layer' in {in_cfg}")
pathlib.Path(out_cfg).write_text(out)
PY
}
...
for spec in "${SPECS[@]}"; do
  ...
  fr_cfg="${CFG_DIR}/config_llama_${model}_tp${tp}_mbz4.yaml"
  cola_cfg="${CFG_DIR}/config_cola_llama_${model}_tp${tp}_mbz4.yaml"
  vanilla_cfg="${cola_cfg}"

  make_cfg "${BASE_FR_CFG}" "${fr_cfg}" "${tp}" "4" \
    "${hidden}" "${intermediate}" "${n_layers}" "${n_heads}" "${n_kv_heads}" \
    "${rank}" "${rank}" "0" "iter_cmp_fr_${model}_tp${tp}_mbz4_%date_%jobid"

  make_cfg "${BASE_COLA_CFG}" "${cola_cfg}" "${tp}" "4" \
    "${hidden}" "${intermediate}" "${n_layers}" "${n_heads}" "${n_kv_heads}" \
    "${rank}" "${rank}" "1" "iter_cmp_cola_${model}_tp${tp}_mbz4_%date_%jobid"

  # Only vanilla CoLA 7B uses activation checkpointing for OOM avoidance at mbz=4.
  if [[ "${model}" == "7B" ]]; then
    vanilla_cfg="${CFG_DIR}/config_cola_llama_${model}_tp${tp}_mbz4_vanilla_recompute.yaml"
    set_recompute_layer "${cola_cfg}" "${vanilla_cfg}" "true"
  fi

  run_cmd "fr_${model,,}_tp${tp}_mbz4" \
    torchrun --nproc_per_node="${tp}" run_train.py --config-file "${fr_cfg}"

  run_cmd "btp_${model,,}_tp${tp}_mbz4" \
    torchrun --nproc_per_node="${tp}" examples/cola/train_cola.py --config-file "${cola_cfg}"

  run_cmd "vanilla_${model,,}_tp${tp}_mbz4" \
    torchrun --nproc_per_node="${tp}" examples/cola/train_vanilla_cola.py --config-file "${vanilla_cfg}"
done
...
result_path = out_dir / "iter_compare_mbz4_table.txt"
...
for model, tp in specs:
    key = model.lower()
    fr, fr_note = parse_avg_ms(out_dir / f"fr_{key}_tp{tp}_mbz4.log")
    btp, btp_note = parse_avg_ms(out_dir / f"btp_{key}_tp{tp}_mbz4.log")
    van, van_note = parse_avg_ms(out_dir / f"vanilla_{key}_tp{tp}_mbz4.log")
