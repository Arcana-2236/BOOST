#!/bin/bash -l
#PBS -N vanilla_cola_60m_1b_token
...
RUN_NAME=Vanilla-$CONFIG_NAME-LR-$LR
TAG=${TAG:-"none"}
if [ "${TAG}" != "none" ]; then
...
mpiexec -n "$NNODES" -ppn 1 --hostfile "$PBS_NODEFILE" \
  bash -lc "
    ...
    torchrun \
      ...
      -- examples/cola/train_vanilla_cola.py --config-file examples/cola/config_${CONFIG_NAME}.yaml \
...
echo "DONE Running Vanilla CoLA Nanotron"